...
func (aq *AuthorityQuery) QueryGroups() *GroupQuery {
...
func (aq *AuthorityQuery) WithGroups(opts ...func(*GroupQuery)) *AuthorityQuery {
	query := (&GroupClient{config: aq.config}).Query()
...
func (aq *AuthorityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Authority, error) {
	...
	if query := aq.withGroups; query != nil {
		if err := aq.loadGroups(ctx, query, nodes,
			func(n *Authority) { n.Edges.Groups = []*Group{} },
			func(n *Authority, e *Group) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthorityQuery) loadGroups(ctx context.Context, query *GroupQuery, nodes []*Authority, init func(*Authority), assign func(*Authority, *Group)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Authority)
	nids := make(map[string]map[*Authority]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(authority.GroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(authority.GroupsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(authority.GroupsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(authority.GroupsPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Authority]struct{}{byID[outValue]: {}}
