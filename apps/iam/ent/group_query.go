...
func (gq *GroupQuery) QueryOrg() *OrgQuery {
	query := (&OrgClient{config: gq.config}).Query()
...
func (gq *GroupQuery) QueryAuthorities() *AuthorityQuery {
	query := (&AuthorityClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAuthorities: gq.withAuthorities.Clone(),
...
func (gq *GroupQuery) WithOrg(opts ...func(*OrgQuery)) *GroupQuery {
	query := (&OrgClient{config: gq.config}).Query()
...
func (gq *GroupQuery) WithAuthorities(opts ...func(*AuthorityQuery)) *GroupQuery {
	query := (&AuthorityClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withAuthorities = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withOrg; query != nil {
		if err := gq.loadOrg(ctx, query, nodes, nil,
			func(n *Group, e *Org) { n.Edges.Org = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := gq.withAuthorities; query != nil {
		if err := gq.loadAuthorities(ctx, query, nodes,
			func(n *Group) { n.Edges.Authorities = []*Authority{} },
			func(n *Group, e *Authority) { n.Edges.Authorities = append(n.Edges.Authorities, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadOrg(ctx context.Context, query *OrgQuery, nodes []*Group, init func(*Group), assign func(*Group, *Org)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Group)
	for i := range nodes {
		if nodes[i].org_groups == nil {
			continue
		}
		fk := *nodes[i].org_groups
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "org_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GroupQuery) loadAuthorities(ctx context.Context, query *AuthorityQuery, nodes []*Group, init func(*Group), assign func(*Group, *Authority)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Group)
	nids := make(map[string]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.AuthoritiesTable)
		s.Join(joinT).On(s.C(authority.FieldID), joinT.C(group.AuthoritiesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.AuthoritiesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.AuthoritiesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Authority](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "authorities" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
