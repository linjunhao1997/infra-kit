...
func (uq *UserQuery) QueryOrg() *OrgQuery {
...
func (uq *UserQuery) QueryNamespace() *NamespaceQuery {
	query := (&NamespaceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOrg:        uq.withOrg.Clone(),
		withGroups:     uq.withGroups.Clone(),
		withNamespace:  uq.withNamespace.Clone(),
...
func (uq *UserQuery) WithOrg(opts ...func(*OrgQuery)) *UserQuery {
	query := (&OrgClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithNamespace(opts ...func(*NamespaceQuery)) *UserQuery {
	query := (&NamespaceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withNamespace = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withOrg; query != nil {
		if err := uq.loadOrg(ctx, query, nodes, nil,
	...
	if query := uq.withNamespace; query != nil {
		if err := uq.loadNamespace(ctx, query, nodes,
			func(n *User) { n.Edges.Namespace = []*Namespace{} },
			func(n *User, e *Namespace) { n.Edges.Namespace = append(n.Edges.Namespace, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadOrg(ctx context.Context, query *OrgQuery, nodes []*User, init func(*User), assign func(*User, *Org)) error {
...
func (uq *UserQuery) loadNamespace(ctx context.Context, query *NamespaceQuery, nodes []*User, init func(*User), assign func(*User, *Namespace)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*User)
	nids := make(map[string]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.NamespaceTable)
		s.Join(joinT).On(s.C(namespace.FieldID), joinT.C(user.NamespacePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.NamespacePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.NamespacePrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Namespace](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "namespace" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
